# Tasks — Anti Dominant Strategy And Active Play
...
## Future Godot Tasks
1. Create future `res://game/rules/anti_dominant_strategy_and_active_play_state.gd` with typed fields for player_id, team_id, Claim family, activity window, and public reason keys.
2. Create future `res://game/rules/anti_dominant_strategy_and_active_play_rules.tres` Resource with caps, thresholds, cooldowns, and FFA/2v2 overrides.
3. Add Signal definitions for `anti_dominant_strategy_and_active_play_changed`, `active_play_window_changed`, `strategy_warning_changed`, and `counter_opportunity_opened` where applicable.
4. Create future `res://ui/hud/anti_dominant_strategy_and_active_play_widget.tscn` as a Control scene using Label, TextureRect, ColorRect, and AnimationPlayer.
