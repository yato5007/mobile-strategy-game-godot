# Tasks — Objective Variety Rotation
...
## Future Godot Tasks
1. Create future `res://game/rules/objective_variety_rotation_state.gd` with typed fields for player_id, team_id, Claim family, activity window, and public reason keys.
2. Create future `res://game/rules/objective_variety_rotation_rules.tres` Resource with caps, thresholds, cooldowns, and FFA/2v2 overrides.
3. Add Signal definitions for `objective_variety_rotation_changed`, `active_play_window_changed`, `strategy_warning_changed`, and `counter_opportunity_opened` where applicable.
4. Create future `res://ui/hud/objective_variety_rotation_widget.tscn` as a Control scene using Label, TextureRect, ColorRect, and AnimationPlayer.
