# Tasks — Strategy Shift Incentives
...
## Future Godot Tasks
1. Create future `res://game/rules/strategy_shift_incentives_state.gd` with typed fields for player_id, team_id, Claim family, activity window, and public reason keys.
2. Create future `res://game/rules/strategy_shift_incentives_rules.tres` Resource with caps, thresholds, cooldowns, and FFA/2v2 overrides.
3. Add Signal definitions for `strategy_shift_incentives_changed`, `active_play_window_changed`, `strategy_warning_changed`, and `counter_opportunity_opened` where applicable.
4. Create future `res://ui/hud/strategy_shift_incentives_widget.tscn` as a Control scene using Label, TextureRect, ColorRect, and AnimationPlayer.
