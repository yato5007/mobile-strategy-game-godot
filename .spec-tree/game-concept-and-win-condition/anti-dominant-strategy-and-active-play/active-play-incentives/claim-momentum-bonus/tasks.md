...
## Future Godot Tasks
1. Create future `res://game/rules/claim_momentum_bonus_state.gd` with typed fields for player_id, team_id, Claim family, activity window, and public reason keys.
2. Create future `res://game/rules/claim_momentum_bonus_rules.tres` Resource with caps, thresholds, cooldowns, and FFA/2v2 overrides.
3. Add Signal definitions for `claim_momentum_bonus_changed`, `active_play_window_changed`, `strategy_warning_changed`, and `counter_opportunity_opened` where applicable.
4. Create future `res://ui/hud/claim_momentum_bonus_widget.tscn` as a Control scene using Label, TextureRect, ColorRect, and AnimationPlayer.
