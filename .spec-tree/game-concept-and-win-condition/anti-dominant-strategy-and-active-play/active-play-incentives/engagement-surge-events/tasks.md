...
## Future Godot Tasks
1. Create future `res://game/rules/engagement_surge_events_state.gd` with typed fields for player_id, team_id, Claim family, activity window, and public reason keys.
2. Create future `res://game/rules/engagement_surge_events_rules.tres` Resource with caps, thresholds, cooldowns, and FFA/2v2 overrides.
3. Add Signal definitions for `engagement_surge_events_changed`, `active_play_window_changed`, `strategy_warning_changed`, and `counter_opportunity_opened` where applicable.
4. Create future `res://ui/hud/engagement_surge_events_widget.tscn` as a Control scene using Label, TextureRect, ColorRect, and AnimationPlayer.
