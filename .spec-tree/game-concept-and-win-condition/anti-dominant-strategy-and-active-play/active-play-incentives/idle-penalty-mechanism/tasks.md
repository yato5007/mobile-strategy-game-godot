...
## Future Godot Tasks
1. Create future `res://game/rules/idle_penalty_mechanism_state.gd` with typed fields for player_id, team_id, Claim family, activity window, and public reason keys.
2. Create future `res://game/rules/idle_penalty_mechanism_rules.tres` Resource with caps, thresholds, cooldowns, and FFA/2v2 overrides.
3. Add Signal definitions for `idle_penalty_mechanism_changed`, `active_play_window_changed`, `strategy_warning_changed`, and `counter_opportunity_opened` where applicable.
4. Create future `res://ui/hud/idle_penalty_mechanism_widget.tscn` as a Control scene using Label, TextureRect, ColorRect, and AnimationPlayer.
