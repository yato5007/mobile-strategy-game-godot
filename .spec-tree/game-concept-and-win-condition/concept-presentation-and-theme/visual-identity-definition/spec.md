# Spec — Visual Identity Definition
...
## Specification
1. Primary design outcome: Define the core visual language: colors, motifs, iconography, typographic direction, and token vocabulary for the Banner of the Majlis identity.
2. Core focus areas: geometric motif system, player color semantics, Arabic-first typography, icon readability.
...
18. `arabic-cultural-mandalas` — Specify how Islamic geometric and Arabic cultural mandala-inspired patterns appear as backgrounds, frames, borders, and subtle ornamental feedback without harming play readability.
19. `color-palette-assignment` — Assign four player colors plus neutral, danger, highlight, and Claim states using an Arabic aesthetic that stays readable on mobile portrait screens.
20. `typography-and-icon-set` — Define Arabic-first font strategy and icon style guide so labels, objective names, Claim values, and player actions feel crafted rather than hardcoded text panels.
