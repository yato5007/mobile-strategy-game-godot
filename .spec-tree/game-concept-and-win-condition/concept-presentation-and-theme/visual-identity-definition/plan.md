# Plan — Visual Identity Definition
...
16. Complete `arabic-cultural-mandalas` because Specify how Islamic geometric and Arabic cultural mandala-inspired patterns appear as backgrounds, frames, borders, and subtle ornamental feedback without harming play readability.
17. Complete `color-palette-assignment` because Assign four player colors plus neutral, danger, highlight, and Claim states using an Arabic aesthetic that stays readable on mobile portrait screens.
18. Complete `typography-and-icon-set` because Define Arabic-first font strategy and icon style guide so labels, objective names, Claim values, and player actions feel crafted rather than hardcoded text panels.
